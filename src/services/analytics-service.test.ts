...
async function runAnalyticsServiceTests() {
  ...
  const mockReflections: Reflection[] = [
    ...
    },
    {
      id: "r3",
      userId: "user1",
      date: new Date("2023-01-03T10:00:00Z"),
      input: "Reflection 3 even more wordy",
      aiSummary: "",
      wordCount: 30,
      createdAt: new Date(),
      updatedAt: new Date(),
    ...
    },
    {
      id: "r4",
      userId: "user1",
      date: new Date("2023-01-03T11:00:00Z"),
      input: "Reflection 4 on same day",
      aiSummary: "",
      wordCount: 15,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  await test("calculateReflectionKPIs - no reflections", async () => {
    ...
    const kpis = await analyticsService.calculateReflectionKPIs(
      "user1",
      "weekly"
    );
    if (kpis.reflectionCount !== 0)
      throw new Error("Reflection count should be 0");
    if (kpis.totalDays !== 7)
      throw new Error("Total days should be 7 for weekly");
    if (kpis.consistencyPercentage !== 0)
      throw new Error("Consistency should be 0");
    if (kpis.averageWordsPerDay !== 0)
      throw new Error("Average words should be 0");
    if (kpis.mostActiveDay !== "Belum ada data")
      throw new Error("Most active day should be 'Belum ada data'");
  });
  ...
  await test("calculateReflectionKPIs - weekly data", async () => {
    ...
    if (kpis.reflectionCount !== 3)
      throw new Error(`Expected 3 reflections, got ${kpis.reflectionCount}`);
    if (kpis.totalDays !== 7)
      throw new Error("Total days should be 7 for weekly");
    // 3 reflections in 7 days = (3/7)*100 = 42.857...
    if (Math.abs(kpis.consistencyPercentage - 300 / 7) > 0.01)
      throw new Error(
        `Expected ~42.86% consistency, got ${kpis.consistencyPercentage}`
      );
    // (10+20+30)/3 = 20
    if (kpis.averageWordsPerDay !== 20)
      throw new Error(
        `Expected 20 average words, got ${kpis.averageWordsPerDay}`
      );
    ...
    if (kpis.mostActiveDay === "Belum ada data" && kpis.reflectionCount > 0)
      throw new Error("Most active day should be determined");
  });
  ...
  await test("calculateReflectionKPIs - monthly data with varied reflections", async () => {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      reflectionsForMonth.push({
        ...
        input: `Monthly reflection ${i}`,
        aiSummary: "",
        wordCount: 10 * ((i % 3) + 1), // 10, 20, 30, 10 ...
        createdAt: new Date(),
        updatedAt: new Date(),
        moodScore: null,
        habitStackingSuggestion: null,
    ...
    if (kpis.reflectionCount !== 15)
      throw new Error(
        `Expected 15 reflections for monthly, got ${kpis.reflectionCount}`
      );
    if (kpis.totalDays !== 30)
      throw new Error("Total days should be 30 for monthly");
    if (Math.abs(kpis.consistencyPercentage - 50) > 0.01)
      throw new Error(
        `Expected 50% consistency, got ${kpis.consistencyPercentage}`
      ); // 15/30
    // Sum of word counts: 5*(10+20+30) = 5*60 = 300. Avg = 300/15 = 20
    if (kpis.averageWordsPerDay !== 20)
      throw new Error(
        `Expected 20 average words for monthly, got ${kpis.averageWordsPerDay}`
      );
    // Mood score related KPIs should be null/N/A by default if not set in mock reflections
    if (kpis.averageMoodScore !== null)
      throw new Error(
        `Expected averageMoodScore to be null, got ${kpis.averageMoodScore}`
      );
    if (kpis.moodScoreTrend !== "N/A")
      throw new Error(
        `Expected moodScoreTrend to be N/A, got ${kpis.moodScoreTrend}`
      );
  });
  ...
  await test("calculateReflectionKPIs - no reflections with moodScore", async () => {
    ...
    if (kpis.averageMoodScore !== null)
      throw new Error(
        `Expected null averageMoodScore, got ${kpis.averageMoodScore}`
      );
    if (kpis.moodScoreTrend !== "N/A")
      throw new Error(`Expected N/A trend, got ${kpis.moodScoreTrend}`);
  });
  ...
  await test("calculateReflectionKPIs - single reflection with moodScore", async () => {
    ...
    if (kpis.averageMoodScore !== 70)
      throw new Error(
        `Expected 70 averageMoodScore, got ${kpis.averageMoodScore}`
      );
    if (kpis.moodScoreTrend !== "Stabil")
      throw new Error(`Expected Stabil trend, got ${kpis.moodScoreTrend}`);
  });
  ...
  await test("calculateReflectionKPIs - multiple reflections, calculates average moodScore", async () => {
    ...
    if (kpis.averageMoodScore !== 70)
      throw new Error(
        `Expected 70 averageMoodScore (60+70+80)/3, got ${kpis.averageMoodScore}`
      );
  });
  ...
  await test("calculateReflectionKPIs - moodScoreTrend 'Meningkat' (weekly)", async () => {
    // @ts-ignore
    mockDb.reflection.findMany = async () => [
      // findMany returns in desc order by default in service
    ...
    if (kpis.averageMoodScore !== 75)
      throw new Error(`Avg mood score error, got ${kpis.averageMoodScore}`);
    if (kpis.moodScoreTrend !== "Meningkat")
      throw new Error(`Expected Meningkat trend, got ${kpis.moodScoreTrend}`);
  });
  ...
  await test("calculateReflectionKPIs - moodScoreTrend 'Menurun' (weekly)", async () => {
    ...
    if (kpis.moodScoreTrend !== "Menurun")
      throw new Error(`Expected Menurun trend, got ${kpis.moodScoreTrend}`);
  });
  ...
  await test("calculateReflectionKPIs - moodScoreTrend 'Stabil' (weekly, insufficient for trend but has data)", async () => {
    ...
    if (kpis.moodScoreTrend !== "Stabil")
      throw new Error(
        `Expected Stabil trend for single point, got ${kpis.moodScoreTrend}`
      );
  });
  ...
  await test("calculateReflectionKPIs - moodScoreTrend 'Stabil' (monthly, <4 data points)", async () => {
    ...
    if (kpis.moodScoreTrend !== "Stabil")
      throw new Error(
        `Expected Stabil trend for <4 points monthly, got ${kpis.moodScoreTrend}`
      );
  });
  ...
  await test("calculateReflectionKPIs - moodScoreTrend 'Meningkat' (monthly, >=4 data points)", async () => {
    ...
    if (kpis.moodScoreTrend !== "Meningkat")
      throw new Error(
        `Expected Meningkat trend monthly, got ${kpis.moodScoreTrend}`
      );
  });
  ...
  await test("generatePersonalizedInsights - should return array of strings", async () => {
    const insights = await analyticsService.generatePersonalizedInsights(
      "user1"
    );
    ...
    if (
      insights.length > 0 &&
      lastInsight &&
      !lastInsight.includes("segera hadir")
    ) {
